...
export function TeachingMethod({ content }: TeachingMethodProps) {
  ...
  return (
    <section id="method" className="py-24 bg-[#F0F8FF] relative overflow-hidden">
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#6EC2F7]/5 rounded-full blur-3xl translate-x-1/3 -translate-y-1/3" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {methods.map((method, index) => (
            <AnimateOnScroll key={index} direction="up" delay={index * 100}>
              <div
                className={`group relative bg-gradient-to-br ${method.bg} rounded-3xl border border-slate-200 p-8 transition-all duration-500 hover:shadow-2xl hover:shadow-[#55B6F6]/10 hover:-translate-y-2 hover:border-[#55B6F6]/30 overflow-hidden ${method.rotate} ${
